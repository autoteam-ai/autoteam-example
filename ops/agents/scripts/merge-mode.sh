...
# 输出 platform：规则集 ai-workflow 生效且允许自动合并，Implementer 开自动合并，平台在评审和检查都通过后合并；
...
repo=$(sed -n 's/^AIWF_REPO=//p' "$root/ops/agents/aiwf.conf" 2>/dev/null | tail -n 1 | tr -d '[:space:]')
...
active=$(gh api "repos/$repo/rulesets" --jq '[.[] | select(.name == "ai-workflow" and .enforcement == "active")] | length' 2>/dev/null)
